...
import { Link, NavLink, useNavigate } from 'react-router-dom'; // Correctly import useNavigate
...
const DELETE_API = 'http://localhost:5164/delete_trainer';

const TrainerDetails = () => {
  const [contact, setContact] = useState([]); // State variable to store trainers
  ...
  const getData = () => {
    ...
    axios.post(URL_API, payload)
      .then(response => {
        ...
        setContact(response.data); // Assuming response.data is an array of trainers
        ...
        <nav className="navbar navbar-default">
          <div className="container-fluid">
            <div className='searchbar'>
              <h2 className='headingsame'>TRAINERS DETAILS</h2>
            </div>
          </div>
        </nav>
        <div className="container-fluid">
                  ...
                  <React.Fragment key={index}>
                    {item.map((value, idx) => (
                      <tr style={{ cursor: 'pointer' }} key={idx}>
                        ...
                        <td>{value[5]}</td>
                        <td>
                          <NavLink to='/edit_trainer'>
                            <Button variant="primary" onClick={() => handleId(value[0])}>Update</Button>
                          </NavLink>
                          <NavLink to="#">
